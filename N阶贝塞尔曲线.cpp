...
Point bezier(vector<Point>, float);
...
public:
	MouseLButton& handleEvent(SDL_Event &e) {
		if (e.type == SDL_MOUSEBUTTONDOWN || e.type == SDL_MOUSEBUTTONUP || e.type == SDL_MOUSEMOTION) {
			/* 获取鼠标坐标 */
			float clientX = e.motion.x, clientY = e.motion.y;

			/* 顶部坐标 */
		
			float x = clientX - diffLeft, y = clientY - diffTop;
			...
			if (e.type == SDL_MOUSEBUTTONDOWN) {
				...
				for (int index = 0; index < clickNodes.size(); ++index) {
					float absX = abs(clickNodes[index].x - x);
					float absY = abs(clickNodes[index].y - y);
					//判断是否在触点范围，设定拖拽
					if (absX < 10 && absY < 10) {
						isDragNode = true;
						dragIndex = index;
			...
			//鼠标移动
				...
				if (isDrag && isDragNode) {
					...
					for (int index = 0; index < clickNodes.size(); ++index) {
						float x = clickNodes[index].x;
						float y = clickNodes[index].y;
						int i = index + 1;
					...
					//	displaybeg();
						...
						*/
						...
						if (index)
						{
							float startX = clickNodes[index - 1].x;
							float startY = clickNodes[index - 1].y;
					...
					if (isPrinted) {
						...
						for (int index = 0; index < bezierArr.size(); ++index) {
							if (index)
							{
								float startX = bezierArr[index - 1].x,
									startY = bezierArr[index - 1].y;
								//绘制线startX,startY->x,y
									float	x = bezierArr[index].x,
										y = bezierArr[index].y;
			...
			else if (e.type == SDL_MOUSEBUTTONUP)//鼠标松开
			{
				...
				if (clickoff - clickon < 200) {
					if (!isPrinted && !isDragNode) {
						...
						if (clickNodes.size())
						{
							float startX = clickNodes[clickNodes.size() - 1].x;
							float startY = clickNodes[clickNodes.size() - 1].y;
		...
		if (_nodes.size() == 1) {
			...
			if (bezierNodes.size() > 1) {
				for (size_t i = 0; i < bezierNodes.size(); ++i) {
					if (i) {
						float startX = bezierNodes[i - 1].x,
...
Point bezier(vector<Point> arr, float t) { //通过各控制点与占比t计算当前贝塞尔曲线上的点坐标
	float x = 0,
		y = 0;
	int n = arr.size() - 1;
	for (int index = 0; index < arr.size(); index++) {
		if (!index) {
			...
			x += (factorial(n) / factorial(index) / factorial(n - index) * arr[index].x * pow((1 - t), n - index) * pow(t, index));
			y += (factorial(n) / factorial(index) / factorial(n - index) * arr[index].y * pow((1 - t), n - index) * pow(t, index));
		}
	}
	return { x,y };
}
...
clearScreen();
		...
		float frameTicks = SDL_GetTicks() - capTimer;
